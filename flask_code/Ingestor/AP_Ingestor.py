...
import os
import traceback
from Ingestor.fetch_data import process_batch_data, read_ddf_from_path
...
class AP_Ingestor:
    ...
    def preprocess_ingest_data(self, ingest_data):
       g.src_ap_cols = ingest_data.columns
       capture_log_message(log_message=f"Src_ap_cols:{g.src_ap_cols}")
    ...
    def ap_accountdocuments(self,transactions):
       ...
       """
       capture_log_message('Calling ap_accountdocuments function')
       columns=['ENTRY_ID']
       ...
       current_table = self.DB.read_table(f'ap_accountdocuments_{self.audit_id}', columns,audit_id=self.audit_id)
       ...
       g.acc_doc_to_id_mapping = dict(zip(final_table['ENTRY_ID'], final_table['ACCOUNT_DOC_ID']))
       # Split ENTRY_ID for database storage
       final_table['ENTRY_ID'] = final_table['ENTRY_ID'].astype(str).str.split('-').str[0]

       # Create reverse mapping: ACCOUNT_DOC_ID -> split ENTRY_ID
       g.swapped_acc_doc_to_id_mapping = dict(zip(final_table['ACCOUNT_DOC_ID'], final_table['ENTRY_ID']))
      
       capture_log_message(log_message='Number of records to ingest:{}'.format(final_table.shape))
       capture_log_message(log_message='Insertion to Accountdoc Started')
       self.DB.upload_data_to_database(final_table, f'ap_accountdocuments_{self.audit_id}')
       capture_log_message(log_message='Insertion to Accountdoc Finished')
    ...
    def ap_transaction(self, transactions):
        ...
        capture_log_message('Mapping started for AP Transaction')
        capture_log_message(f"lenth of data frame before mapping : {len(transactions)}")
        capture_log_message(f"Fetched details - location_table:{len(location_table)}, user_table:{len(user_table)}, company_table:{len(company_table)}, coa_table:{len(coa_table)}, vendor_table:{len(vendor_table)}, approval_user:{len(approval_user)}")
        ...
        # For all  columns created for mapping, check for null values and log them
        mapping_columns = ['ACCOUNT_ID','ACCOUNT_CODE','ENTERED_BY','POSTED_BY','FMSUSER_CODE','COMPANYID','COMPANY_CODE','COMPANY_CODE_NAME',
                            'LOCATION_ID','LOCATION_CODE','VENDORID','VENDORCODE','SUPPLIER_NAME']
        for col in mapping_columns:
            
            null_count = transactions[col].isna().sum()
            if null_count > 0:
                capture_log_message(f"Mapping Warning: Column '{col}' has {null_count} unmapped values.")

            else:
                capture_log_message(f"Mapping Info: Column '{col}' has all values mapped successfully.")
        ...
        pipe_obj.set_data(f'audit_{self.audit_id}',aptransaction_df)
        ...
        self.DB.upload_data_to_database(transactions, f'ap_transaction_{g.audit_id}')
    ...
    def run_job(self):
        ...
        try:
            ...
            existing_data = self.DB.read_table(f'ap_transaction_{self.audit_id}', columns=['TRANSACTION_CODE'],audit_id=self.audit_id)
            capture_log_message(log_message='Duplicate check started for transaction data')
            transactions = ingest_data.loc[~ingest_data['TRANSACTION_ID_GA'].isin(existing_data['TRANSACTION_CODE'])].copy()
            capture_log_message(log_message=' Shape of AP_transactions data after excluding duplicate:{}'.format(transactions.shape))
            ...
            g.ap_ingestion_end_time = ingestion_end_time
