# Optimization_module/Early_Posted_Invoices/Componenets/transformers.py
import pandas as pd
...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_ = X.copy()
        X_["DUE_PAYMENT_DIFFERENCE"] = self._calculate_due_payment_diff(X_)
        X_['EARLY_PAYMENT'] = np.where((X_['DUE_PAYMENT_DIFFERENCE']>0),1,0)
        ...
        X_['PAYMENT_QTR'] = X_['PAYMENT_DATE'].dt.quarter.fillna(0).astype(int)
        X_['PAYMENT_DAY']=X_['PAYMENT_DATE'].dt.day.fillna(0).astype(int)
        X_['PAYMENT_MONTH']=X_['PAYMENT_DATE'].dt.month.fillna(0).astype(int)
