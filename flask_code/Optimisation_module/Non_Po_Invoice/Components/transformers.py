# Optimization_module/Non_Po_Invoice/Componenets/transformers.py
import pandas as pd
...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_ = X.copy()
        X_["HAS_PO_INVOICE"] = np.where(X_['PURCHASE_ORDER_NUMBER'].isna(),0,1)
        X_["LOG_AMOUNT"]            = self._calculate_log_amount(X_)
        # print("inside non po fearure transform")
