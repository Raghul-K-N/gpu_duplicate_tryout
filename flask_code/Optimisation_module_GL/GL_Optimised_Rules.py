from Optimisation_module_GL.gl_optimisation_utils import is_optimisation_model_available, predict_rules_scores
from code1.logger import capture_log_message
import pandas as pd
import numpy as np


RULES = [   'BLANK_JE', 'UNUSUAL_ACCOUNT_PAIRING', 'SUSPICIOUS_KEYWORDS',
            'POSTS_HOLIDAYS', 'POSTS_WEEKEND', 'NEXT_QTR_POSTING',
            'SAME_USER_POSTING', 'POSTS_NIGHT' ]
...
def optimise_rule_scores(Scored_DF: pd.DataFrame, df_rules_scored: pd.DataFrame, rule_weights: dict, opt_rule_weights: dict) -> pd.DataFrame:
    ...
    """
    capture_log_message("Starting optimisation of GL rule scores…")
    ...
    capture_log_message("Completed optimisation of GL rule scores.")
    return Scored_DF
...
def optimised_doc_lvl_scores_calculation(df: pd.DataFrame, rule_weights: dict) -> pd.DataFrame:
    ...
    rules_accountdoc = df.groupby('ACCOUNTDOCID',as_index=False).agg(agg_dict)
...
def optimised_blended_score_calculation(Scored_DF):
    ...
    Scored_DF['OPTIMISED_BLENDED_RISK_SCORE_RAW'] = Scored_DF['OPTIMISED_RULES_RISK_SCORE_RAW']  
...
def optimised_acc_doc_lvl_rule_scores(df: pd.DataFrame, rule_weights: dict) -> pd.DataFrame:
    ...
    rules_accountdoc = optimised_blended_score_calculation(rules_accountdoc)
