# Parameters/UDC/__init__.py
"""
UDC (Delegation of Authority) Field Validator - Phase 1 Implementation
...
from invoice_verification.constant_field_names import UDC, GL_ACCOUNT_NUMBER

class Udc:
    """
    UDC parameter validator
    ...
    def __init__(self, invoice_ocr, voucher_ocr, sap_row: SAPRow, vendors_df=None, vat_df=None, gl_accounts_df=None):
        ...
        log_message(f"UDC GL Accounts List: {self.gl_accounts_list} (Source: {self.gl_account_source})")
    ...
    def main(self) -> dict:
        """
        Main method to validate UDC
        Returns:
            Dict with standardized output format
        """
        try:
            log_message(f"Processing UDC for transaction: {self.sap_row.transaction_id}")
            # Route to region-specific validation
            result = self._validate_udc()
            
            return result
            
        except Exception as e:
            log_message(f"Error in UDC validation: {str(e)}", error_logger=True)
    ...
    def _validate_udc(self) -> dict:
        """Route to region-specific validation logic"""
    
        if self.region in ['NAA', 'NA']:
            from .na import validate_naa_udc
            return validate_naa_udc(
            ...
            from .emeai import validate_emeai_udc
            return validate_emeai_udc(
                sap_row=self.sap_row,
                voucher_text=self.voucher_text,
                gl_accounts_list=self.gl_accounts_list
            )
            ...
            from .apac import validate_apac_udc
            return validate_apac_udc(
                sap_row=self.sap_row,
                voucher_text=self.voucher_text,
                gl_accounts_list=self.gl_accounts_list
            )
            ...
            from .la import validate_latam_udc
            return validate_latam_udc(
                sap_row=self.sap_row,
                voucher_text=self.voucher_text,
                gl_accounts_list=self.gl_accounts_list
            )
            ...
            raise ValueError(f"Unsupported region for UDC validation: {self.region}")