# Parameters/UDC/utils.py
# ============================================================================
"""
Utility functions for UDC validation
"""
...
def check_udc_approved_in_vim(vim_comments: Optional[str]) -> bool:
    """
    ...
    approved = 'APPROVED' in vim_comments.upper()
    log_message(f"UDC approval in VIM comments: {approved}")
    return approved


def check_udc_approved_in_eml(eml_lines: Optional[str], sap_invoice_number: Optional[str]) -> bool:
    """
    ...
    for line in eml_lines.split('\n'):
        ...
        if 'APPROVED' in line_upper:# and str(sap_invoice_number).strip() in line:
            log_message("UDC: Approved line found in EML")
            ...
            if invoice_number_present:
                log_message(f"UDC: UDC approval with invoice {sap_invoice_number} found in EML")
                return True if invoice_number_present else False
    
    log_message(f"UDC: UDC approval with invoice {sap_invoice_number} NOT found in EML")
    return False
...
def check_approval_status(opentext_user_id: str, app_action: str, 
    ...
    """
    if source == 'voucher':
        log_message(f"UDC: Checking voucher approval - User: {opentext_user_id}, App Action: {app_action}")
        ...
        approved = True
        log_message(f"UDC: Voucher approval check - Always Default Approved: {approved}")
        return approved
    
    # Try VIM if voucher didn't have approval
    if vim_comments:
        log_message(f"UDC: Checking VIM approval - User: {opentext_user_id}")
        if check_udc_approved_in_vim(vim_comments):
            log_message(f"UDC: VIM approval found - User: {opentext_user_id}")
            return True
        log_message(f"UDC: VIM approval not found, checking EML...")
    
    # Try EML if VIM didn't have approval
    if eml_lines:
        log_message(f"UDC: Checking EML approval - User: {opentext_user_id}")
        if check_udc_approved_in_eml(eml_lines, sap_invoice_number):
            log_message(f"UDC: EML approval found - User: {opentext_user_id}")
            return True
        log_message(f"UDC: EML approval not found")
    
    log_message(f"UDC: No approval found in any source for User: {opentext_user_id}")
    return False
...
def clear_data_cache():
    ...
    global _diamond_data_cache, _pro_vr_doa_data_cache
    _diamond_data_cache = None
    _pro_vr_doa_data_cache = None
    log_message("UDC data cache cleared")
